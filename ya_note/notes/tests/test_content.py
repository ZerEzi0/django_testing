...
class YaNoteContentTestCase(unittest.TestCase):
    def setUp(self):
        self.client = Client()
        ...
        self.note2 = Note.objects.create(title="User2's Note", text="Text for user2", slug="user2-note", author=self.user2)
    ...
    def test_note_form_in_creation_and_edit_pages(self):
        ...
        response = self.client.get(reverse('notes:edit', kwargs={'slug': self.note1.slug}))
        self.assertEqual(response.status_code, 200)
        self.assertIsInstance(response.context['form'], NoteForm)

    def test_notes_are_filtered_by_user(self):
        self.client.login(username='user1', password='password1')
        response = self.client.get(reverse('notes:list'))
        self.assertEqual(response.status_code, 200)
